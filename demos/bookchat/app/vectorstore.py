...
class VectorStore:
    ...
    def _create_index_if_not_exists(self) -> None:
        ...
        if exists.meta.status == 200:
            logger.debug(
                f"Index {self.index} already exists. Skipping creation.")
            return
        ...
        if self.retrieval_strategy.needs_inference():
            ...
            if not self.num_dimensions and self.embedding_service:
                vector = self.embedding_service.embed_query(
                    "get num dimensions")
        ...
        if self.metadata_mappings:
            metadata = mappings["properties"].get(
                "metadata", {"properties": {}})
            for key in self.metadata_mappings.keys():
                if key in metadata:
                    raise ValueError(
                        f"metadata key {key} already exists in mappings")
    ...
    def max_marginal_relevance_search(
        ...
        *,
        ...
        if query_embedding:
            ...
            if not query:
                raise ValueError(
                    "specify either query or query_embedding to search")
                ...
                raise ValueError(
                    "specify embedding_service to search with query")
